...
## 検索結果に出る「サイト名」
Google は検索結果のサイト名を**ルートURI（＝ `/`）に置かれた `WebSite` 構造化データ**から決める。
`/{言語}/` に置いたものはホームページとして扱われない（公式: ホームページとはドメインまたは
サブドメインのルートURI。`https://example.com/de/index.html` はホームページではない）。

そのため **ルート `/` は転送せず、中身を 200 で返す**（`index.php` が `$URL_DEPTH = 0` で
`includes/home.php` を呼ぶ）。以前は `/{言語}/` へ 302 していたが、それだと構造化データを
拾えず、相乗り先の親ドメイン（`genstrings.sakura.ne.jp` → さくらインターネット）の名前が
検索結果に出てしまっていた。

* **正規URL（canonical）は `/{言語}/` のまま。** ルートは同じ内容の重複ページとして扱う。
  Google のサイト名の項に「ホームページが重複している場合は、正規ページだけでなく重複する
  すべてのページで同じ構造化データを使う」とあるので、正規化を崩さずにルートへ置ける。
* `WebSite` の **`url` は必ずサイトのルート**（`$SITE_ROOT`）。言語別URLを入れると
  そこをホームページと見なしてもらえない。`name` は `APP_NAME`。
* サイト名の材料は `WebSite` 構造化データ ＞ `og:site_name` ＞ `<title>` ＞ 見出し の順に見られる。
  **4つの表記をそろえること**（今はすべて `GEN strings` 始まり）。
* ルートは Accept-Language で言語が変わるので `Vary: Accept-Language` を返す。
  `?lang=` の明示指定が最優先、次に Accept-Language、どれでもなければ既定言語。
* `favicon.ico` をルートに置いてある。検索結果のアイコンで `/favicon.ico` を見に来るため
  （`<link rel="icon">` も併せて出している）。
* **効かないこともある。** サイト名は最終的に Google が決めるもので、指定は「候補」にすぎない。
  とくに共用ホスティングのサブドメイン（`*.sakura.ne.jp`）は親ドメインの名前が強く、
  上を全部やっても変わらない場合がある。確実にしたいなら**独自ドメインへ移す**しかない。
