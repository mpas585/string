...
return [
  ...
  /* 指板に表示する開放弦からの半音数の上限（src/fingerboard.js FB.maxOff と同値）。
     実物の指板の先端に合わせる。4/4チェロの指板長は 578〜583mm（Strobel 580 /
     Goldblatt 578-583）、弦長は 690〜700mm（Strobel の寸法表は 695）なので
        580 / 695 = 83.5%  →  12 * log2(1 / (1 - 0.835)) = 31.1  →  off 31
     off31 はナットから 579.0mm。資料値の指板長とほぼ一致する。
     ※ 指板の先端は log の性質上、わずかな長さの差が大きな音程差になる。
        Aitchison の目安「指板長＝弦長の85%」を採るなら 33 になる（＝2半音ぶん違う）。
     ※ 白鳥の最高音 D6（A線 off29 = 564.8mm）は範囲内。 */
  'max_off'  => 31,
  ...
  /* ネックとボディの接合部（開放弦からの半音数。src/fingerboard.js FB.bodyOff）。
     測る基準は「ナット → 表板の上端（ネックがボディに接する点）」＝ネック長 280mm。
     出し方が2通りあって、わずかに答えが違う:
        製作比 7:10（ネック280 : ボディストップ400）から  12*log2(17/10)      = 9.19
        実寸 280mm ÷ 弦長 695mm = 40.3% から              12*log2(1/0.597)   = 8.93
     280+400=680 なのに公表弦長が 690〜700mm あるのは、駒で弦が立ち上がるぶん
     実際の振動長が2直線の和より長くなるため。両者の間をとって off 9 とする。
     off9 はナットから 281.8mm ＝ 資料のネック長 280mm とほぼ一致。
     ※ ポジション標識 V（off9）と同じ高さに来るが、これは正しい。4thポジション
        （1指E=off7 / 4指G=off10）で親指がネックの付け根に当たる、という
        奏法上の説明と実寸が一致する＝接合部は4thポジションの手の中にある。
     ※ ヴァイオリン・ヴィオラはネック:ストップ＝2:3 なので off ≈ 8.8。 */
  'body_off' => 9,
