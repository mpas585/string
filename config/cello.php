...
return [
  ...
  /* 指板に表示する開放弦からの半音数の上限（src/fingerboard.js FB.maxOff と同値）。
     実物の指板長に合わせる。4/4チェロの指板は約580mm、弦長は680mm なので
        12 * log2(680 / (680-580)) = 33.19  → off 33（ナットから 578.9mm）
     これで接合部(off 9.19 = 280mm)が板の 48.4% に来て、実物の見た目と一致する。
     ※ 30 だと板が 559.8mm しかなく、接合部がちょうど板の中央(50.0%)に来てしまう。
     ※ 白鳥の最高音 D6（A線 off29）は変更後も範囲内。 */
  'max_off'  => 33,
  ...
  /* ネックとボディの接合部（開放弦からの半音数。src/fingerboard.js FB.bodyOff）。
     4/4チェロの寸法は ネック長 280mm : ボディストップ 400mm ＝ 7:10（弦長 680mm）。
     接合部は駒から見て 400mm 手前なので、開放弦からの半音数は
        12 * log2(680 / 400) = 9.186
     ナットから接合部までの比でいうと 280/680 = 0.4118。
     ※ 9 にすると 0.4054 となり、接合線がポジション標識 V（off=9）に重なってしまう。
        実際の接合部は V よりわずかに下（駒側）にある。 */
  'body_off' => 9.19,
